...
TASK_C_VIEW_STATEMENTS = [
    ...
    GROUP BY psm.plant_id, p.name, EXTRACT(YEAR FROM psm.time), EXTRACT(MONTH FROM psm.time)
    ;
    ...
    CREATE VIEW energy_flow AS
        ...
        JOIN public.energy_delivered ed
             ON ed.pwr_plant_id = eu.id
            AND ed.year = ppp.year
            AND ed.month = ppp.month
        ORDER BY ppp.power_plant_source, ppp.year, ppp.month
    ;
    """,
]
